...
// 客户端埋点
export function buryPointMessage (obj) {
  Bridge.callHandler('buryPointMessage', obj, (res) => {
    console.log(res, '埋点成功')
  })
}
// 请求埋点
export function buryLogMessage (obj) {
  Bridge.callHandler('buryLogMessage', obj, (res) => {
    console.log(res, '请求埋点成功')
  })
}
// 客户端退到后台停止所有音频
export function pauseAudioWhenBack () {
...
export function showLoading (msg) {
  Bridge.callHandler('showLoading', msg)
}
...
export function showDevicePerfect () {
  Bridge.callHandler('showDevicePerfect', {}, (res) => {
    console.log('调用五星好评成功')
  })
}
// 隐藏客户端导航栏
export function hideNativeNavBar () {
  setTimeout(() => {
    Bridge.callHandler('hideNativeNavBar', {}, (res) => {
      console.log('隐藏客户端导航栏成功')
    })
  })
}
// 隐藏客户端状态栏
export function hiddenStatusBar () {
  return new Promise((resolve, reject) => {
    Bridge.callHandler('hiddenStatusBar', {}, (res) => {
      console.log('隐藏客户端状态栏成功,状态栏高：' + res)
      resolve(res)
    })
  })
}
// 分享
export function showShare (params) {
  return new Promise((resolve, reject) => {
    Bridge.callHandler('showShare', params, (res) => {
      console.log('分享' + res)
      resolve(res)
    })
  })
...
// 打开优秀配音页面
export function showGreateDubList (params) {
  return new Promise((resolve, reject) => {
    Bridge.callHandler('showGreateDubList', params, (res) => {
      console.log('打开优秀配音页面' + res)
      resolve(res)
    })
  })
}
// 打开某个学生优秀配音
export function showExcellentDub (params) {
  return new Promise((resolve, reject) => {
    Bridge.callHandler('showExcellentDub', params, (res) => {
      console.log('打开某个学生优秀配音' + res)
      resolve(res)
    })
  })
}
...
export function getParam () {
  const isHash = location.hash
  var paramStr = isHash ? decodeURIComponent(location.hash) : decodeURIComponent(location.search)
  if (paramStr.indexOf('?') !== -1) {
    var str = isHash ? paramStr.split('?')[1] : paramStr.substr(1)
    return `?${str}`
  } else {
    return ''
  }
